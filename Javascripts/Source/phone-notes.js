...
function getPhoneNotes(stype)
{
	...
	if (stype === "notes1") {
		...
		// Schedules
		function addSchedule(nm, desc, flg, extra) {
			return '<span style="position:relative;top:0.5em"><b>' + nm + '</b> ' + desc + (flg === true ? (extra === undefined ? " After these hours " + per.getHeShe() + " is in " + per.getHisHer() + " home at " + per.getPersonAddress() : extra) + '<br>' : '<br>') + '</span>';
		}
		...
		// Monique
		findPerson("Monique");
		if (per.other > 0) sp += addSchedule("Monique", "works from 7am to 8pm 7 days a week at the Library.", per.isCharmedBy());
		// Ms MsTitus
		findPerson("MsTitus");
		if (per.other > 0) sp += addSchedule("Ms. Titus", "works from 7am to 8pm 7 days a week at the Library.", isPlaceKnown("MsTitussHouse"));
		// Nina
		findPerson("Nina");
