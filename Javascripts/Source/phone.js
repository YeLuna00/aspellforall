...
// NOTE: currently max allowed is set in gameState.nSMSLimit, currently 416
var arSMS;
// Assigned id's
// Ms. Titus	: 1, 2, 3, 4, 5
...
// Sarah			: 90, (Conspiracy only) 91, 92, (all) 99
...
function receiveCall(from, txt, nt)
{
	...
	setComments('<div class="' + getConverseBubbleClass() + '"><table><tr><td style="width:15%;vertical-align:top"><img src="UI/phonecall.png" style="width:95%" alt="Phone Call"></td><td><p>' + (from !== '' ? '<b>Phone call from ' + from + '</b><br>' : '') + txt + '</p></td></tr></table>', txt.indexOf("gblock") == -1);
}

/* Replaces with version in Person object, left for now in case we need to restore
// Make a call/SMS
...
{
	if (Place < 2) return;
	if (gameState.nRightBarState < 3) showRightBar(gameState.nRightBarState + 2);
...
function usePhone(stypein, no)
{
	...
	if (stype === "" || stype === undefined || stype == "alarm" || stype == "memory" || stype == "sms" || stype == "sendsms" || stype == "clear" || stype == "delete" || stype == "addressbook" || stype == "apps" || stype == "game" || stype == "help" || stype == "games") {
...
function getPhoneContents()
{
	...
	if (stype == "notes1" || stype == "notes2" || stype == "notes3" || stype == "notes4" || stype == "notes5") s = getPhoneNotes(stype);  // Notes		
	else if (stype === "" || stype === undefined || stype == "memory" || stype == "alarm" || stype == "sms" || stype == "clear" || stype == "delete" || stype == "addressbook" || stype == "apps" || stype == "game" || stype == "help" || stype == "games") {
		...
		if (stype == "sms") {
			...
			if (no === '') {
				...
				s += '</div>' + (cnt === 0 ? '<br>' : '');
				if (p.isPhoneable(true) && p.charmed !== -1) {
				...
				for (j = 0; j < par.length; j++) {
					...
					if (ar.length == 1 && (!pa.isPhoneable(true) || pa.charmed === -1)) continue;
			...
			nUnreadSMS = 0;
			s += '<br></div>' + addOptionLink("string", "Clear", "usePhone(\'clear\')", undefined, "position:absolute;top:79%;top:calc(86% - 2.75em);width:20%;left:8%;margin-right:auto;border-left-width:0;padding-top:0.25em;padding-bottom:0.25em;line-height:1em;background-color:#5E5E5E;color:#FFFFFF;margin-top:1em") + " " +
			...
			s += '<img style="position:absolute;width:100%;height:100%;display:inline-block;border-width:0;border-style:none;top:0;left:0;margin:0px 0px 0px 0px;padding:0" src="UI/phone3p.png" alt="phone">' +
				...
				'</tr></table></div>' +	addOptionLink("string", "Done", "usePhone()", undefined, "position:absolute;top:79%;top:calc(86% - 2.75em);width:36%;left:51%;margin-right:auto;border-left-width:0;padding-top:0.25em;padding-bottom:0.25em;line-height:1em;margin-top:1em");
			...
			s += '<img style="border:none;position:absolute;width:100%;height:100%;border-width:0;border-style:none;top:0;left:0;margin:0px 0px 0px 0px;padding:0" src="UI/phone3p.png" alt="phone">' +
				'<p style="position:absolute;z-index:75;top:7.2vh;left:5%;color:' + wpt + (perYou.extra[7] == 1 ? ";text-shadow:-1px 0px black, 0px 1px white, 1px 0px white, 0px -1px white;" : ";text-shadow:-1px 0px black, 0px 1px black, 1px 0px black, 0px -1px black;") + ';font-size:large;width:89%;text-align:right"><b> ';
			...
			s +='<p style="position:absolute;top:76vh;left:11%;width:80%;font-size:small"><a href="javascript:usePhone(\'apps\')"><img id="appsicon" src="UI/apps.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Photos" title="Apps"></a></p>' +
				 '<p style="position:absolute;top:76vh;left:27%;width:80%;font-size:small"><a href="javascript:usePhone(\'addressbook\')"><img id="addressicon" src="UI/addressbook.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Photos" title="Address Book"></a></p>' +
				 '<p style="position:absolute;top:76vh;left:43%;width:80%;font-size:small"><a href="javascript:usePhone(\'photos\')"><img id="cameraicon" src="UI/camera.png" style=height:8vh;width:19%;margin-right:0;margin-left:0" alt="Photos" title="View Photos"></a></p>' +
				 '<p style="position:absolute;top:76vh;left:59%;width:80%;font-size:small"><a href="javascript:usePhone(\'notes2\')"><img id="notesicon" src="UI/notes.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Notes" title="View Notes"></a></p>' +
				 '<p style="position:absolute;top:76vh;left:75%;width:80%;font-size:small"><a href="javascript:usePhone(\'map\')"><img id="mapicon" src="UI/map.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Map" title="Local Map"></a></p>';
			if (perYourBody.checkFlag(15)) s += '<p style="position:absolute;top:calc(67vh + 4px);left:75%;width:80%;font-size:small"><a href="javascript:usePhone(\'games\')"><img id="gamesicon" src="UI/games.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Game" title="Play a Game"></a></p>';
			else s += '<p style="position:absolute;top:calc(67vh + 4px);left:75%;width:80%;font-size:small"><a href="javascript:usePhone(\'games\')"><img id="gamesicon" src="UI/games.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Game" title="Play a Game"></a></p>';
			s += '<p style="position:absolute;top:calc(58vh + 4px);left:75%;width:80%;font-size:small"><a href="javascript:usePhone(\'help\')"><img id="helpicon" src="UI/help2.png" style="height:8vh;width:19%;margin-right:0;margin-left:0" alt="Help" title="Help"></a></p>';
		...
		s = '<script>document.onkeypress = stopRKey;initLightbox();</script>' +
			...
			getMapHTML("100%", "100%",
				function(id) {
					if (!perYou.checkFlag(66) || id == 317) return '';
