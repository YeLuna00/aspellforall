...
function initialiseCherry()
{
	...
	per.showEventPopup = function()
	{
		...
		if (!this.checkFlag(25) && this.getCharmedLevel() == 4 && this.hoursCharmed() > 23) {
			this.setFlag(25);
	...
	per.showEvent = function()
	{
		...
		if (sType == "charmcherry1") {
			...
			md.write(
				' She addresses you,</p>' +
				'<p>"Something you said, I know and it will in a few minutes make you your complete and subvient slave"'
			);
	...
	per.addPersonPhoneCall = function() {
		...
		if ((nTime - this.charmedTime) > 8 && isMorning() && !this.checkFlag(5)) {
			// First morning after being charmed
			if (this.makeCall(true, 390)) {
				this.setFlag(5);
			}
		}
		if ((nTime - this.charmedTime) > 48 && isMorning() && this.checkFlag(5) && !(this.checkFlag(6) || this.checkFlag(7))) {
			// First morning after being charmed
