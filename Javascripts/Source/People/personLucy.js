...
function initialiseLucy()
{
	...
	per.showPersonTextHere = function(md)
	{
		if (Place == 144 && sType === "" && isShopOpen(2) && !isSpellKnown("Charm") && !this.isCharmedBy("Demon") && !this.checkFlag(4)) {
			// First glimpse
			if (this.dress === "") {
				this.pickModel('You notice a game being played on the oval, not a school or class thing. You see two officials discussion something. You sort of recognise one. Is she the one with...', "lucy0", "Lucie", "Whitney", "reddish hair", "darker brown hair", "", "Who is that unpire?");
				return true;
			}
			this.setFlag(4);
			md.write(
