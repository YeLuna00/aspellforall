...
function RepliesBeasley(nR)
{
	...
	else if (nR == 192)
	{
		...
		addComments(
			'"Rumour has it," he says with an overly dramatic sigh, "that mana can be found at the stones in the Wild Ranges, to the north of the park. Oh, and usually it is stored in special stones of some kind. " With a piece of chalk he shows you a peculiar way to <b>use</b> these stones for mana.</p>' +
			'<p>He signs, "Now please let me get on with my work."'
		);
...
function initialiseBeasley()
{
	...
	perBeasley.showEventPopup = function()
	{
		...
		if (sType == "pickupbeasleypaper") {
			...
			showPopupWindow("Lecture Notes",
				...
				"Mind you, that recent problem you had reset your phone, so all previous notes were lost, you will rapidly make up for that! You will have to refill your address book as well!",
				"showHintIconP('Phone')"
			);
			return true;
		}
	...
	perBeasley.showEvent = function()
	{
		...
		if (Place == 70 && !this.checkFlag(1)) {
			...
			md.write(
				"<p>You hear a teacher call you and you walk into his classroom, he is your least favourite teacher, Mr Beasley. It is not that he is nasty or harsh, you just cannot help to feel he is a bit slimy; a creep.</p>" +
		...
		if (Place == 11) {
			...
			if (sType == "beasleysexfun") {
				...
				startQuestions();
				addLinkToPlace(md, "back to the classroom", 11);
				WritePlaceFooter(md);
				return true;				
			}
			...
			if (sType == "malebj") {
				...
				md = WritePlaceHeader();
				this.showPersonRorX("beasley8b.jpg");
			...
			if (sType == "femalebj") {
				...
				md = WritePlaceHeader();
				this.showPersonRorX("beasley8g.jpg");
			...
			if (sType == "femaleservice") {
				// Sex with Miss Beasley
				md = WritePlaceHeader();
				if (this.getCharmedLevel() == 1) this.charmThem(2);

				if (isExplicit() && this.dress == "Bimbo1" && !perYou.isMaleSex()) this.showPerson("beasley8g.jpg");
				else if (perYou.isMaleSex()) this.showPersonRorX("beasley8b.jpg");
				else this.showPerson("beasley8g.jpg");
				...
				startQuestions();
				if (canMoveDavyToHotel2())	addQuestionCO(md, 'ask Beasley where Davy has gone', "MrBeasley", 5501);
				addLinkToPlace(md, "exit the room?", 70);
				WritePlaceFooter(md);
				return true;
			}
		...
		if (sType == "transformbimbo1") {
			...
			showPopupWindow("Transformation",
				...
				'<p>She asks you raggedly, "What did you do? Was this...umm...what is it called...Ali Massive...Al..Mass...yeah that\'s it." and she laughs, but it sounds nearer a giggle. She tries to cover up her exposed breasts but it looks more like she is offering them to you. You tell her it was and ask how she is feeling,</p>' +
		...
		if (sType == "transformbimbo2") {
			...
			showPopupWindow("Transformation",
				...
				'<p>You drag her back into the classroom and ' +
				(perYou.isMaleSex() ? 'fuck her as she demanded, hard and fast in her new pussy' : 'lick her for a few moments until she explosively climaxes. You then make her lick you and she furiously masturbates as she does') +
		...
		if (sType == "transformbondage1") {
			...
			md = WritePlaceHeaderNIP(true, '', 'black');
			showPopupWindow("Transformation",
				'<img src="Images/People/Beasley/Bondage1/beasley-transform.jpg" class="imgpopup" alt="TG">' +
	...
	if (sType == "transformbondage2") {
			...
			md = WritePlaceHeaderNIP(true, '', 'black');
			showPopupWindow("Transformation",
				'<img src="Images/People/Beasley/Bondage2/beasley-transform.jpg" class="imgpopup" alt="TG">' +
				'<p>You cast the spell and ' + this.getPersonName() + ' gasps and as you watch ' + this.getHisHer() + ' chest swells and hair grows from ' + this.getHisHer() + ' previously bald head. ' + capitalize(this.getHisHer()) + ' figure changes to a slim female form and ' + this.getHeShe() + ' unbuttons ' + this.getHisHer() + ' shirt in disbelief exposing <i>her</i> breasts. She drops her pants showing a delightful ass and you see a glimpse of a new pussy.</p>' +
				'<p>She asks you incredulously, "You cast Al Mass on me? How did you learn it, it is lost, a spell of legend, not even Kurndorf was supposed to know it!". You tell her yes it was and she advances, intent on hitting you despite the charm spell. You call out for her to stop and instantly she stops moving, a look of disbelief on her face. She demands, "What is this, why can\'t I move?"</p>' +
				'<p>You have no idea, and for an experiment you tell her to expose herself more. She immediately strips more and looks at you angrily. It seems she must obey your physical orders, interesting! You tell her to do what she wants, and she quickly retreats behind her desk. She seems to be able to act normally now, but before despite being charmed she tried to hit you before! To be safe you tell her, "You can never hit me, harm me or work against me in any way" and she answers with her eyes glowing slightly,</p>' +
				'<p>"Yes ' + perYou.getMaster() + '"</p>' +
				'<p>She still looks defiant and you decide to reinforce her new found submission. You tell her sto strip naked, and you quick go to a supply closet and get some ropes, and proceed to tie her up, and tell her she cannot free herself, only you can untie her.</p>'
			);
			setQueryParams("");
			WritePlaceFooter(md);
			return true;
	...
	perBeasley.showPersonChat = function(md)
	{
		...
		else if (this.getPersonGender() == "woman") addLinkToPlaceO(md, "fuck " + nm, 11, 'type=femaleservice');
	...
	perBeasley.handleItem = function(no, cmd)
	{
		...
		// Casting the transform spell
			...
			if (this.isHere()) {
				...
				setCommentsNoClick('<div class="' + getConverseBubbleClass() + '" style="cursor:default"><table><tr><td width="80%"><p>');
				if (this.getPersonGender() == "man") addComments('You remember Catherine\'s desire for revenge and you feel you should try to control Mr. Beasley more. Catherine wanted him "fucked up" and with this in mind you cast the spell. As you do different thoughts cross your mind, different things Catherine had said or shown you at times,</p>');
				else addComments('You wonder about changing your control over Miss. Beasley. With Catherine\'s goal of making her "fucked up" you cast the spell. As you do different thoughts cross your mind, different things Catherine had said or shown you at times,</p>');
				if (!this.checkFlag(10)) addOptionLink("comments", '...huge breasted bimbo\'s', "ClearComments();dispPlace(" + Place + ",'type=transformbimbo1')");
				if (!this.checkFlag(11)) addOptionLink("comments", '...sexy bimbo sluts', "ClearComments();dispPlace(" + Place + ",'type=transformbimbo2')");
				if (!this.checkFlag(12)) addOptionLink("comments", '...that bondage image of Catherine', "ClearComments();dispPlace(" + Place + ",'type=transformbondage1')");
				if (!this.checkFlag(13)) addOptionLink("comments", '...that bondage image of Catherine but stricter', "ClearComments();dispPlace(" + Place + ",'type=transformbondage2')");
				addComments('</td><td width="20%">' + this.addPersonFace(false, '95%') + '</td></tr></table>');
				return "handled";
			}
		}
