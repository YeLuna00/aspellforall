...
function initialiseCatherineRoss()
{
	...
	per.getPersonAddress = function(n) { return n ? 438 : "5 Cherise Rd, Glenvale"; };
		...
		if (sType == "catherinebusy" || (!this.checkFlag(9) && (Place == 10 || Place == 72) && nFromPlace != Place && nTime > 288 && Math.random() < 0.25)) {
			if (!checkPersonFlag("MrBeasley", 3)) {
				if ((Place == 10 && wherePerson("Monique") != 10) || (Place == 72 && wherePerson("MrsTanika") != 72)) {
					...
					startQuestions();
					addLinkToPlace(md, "leave them alone", 70);
					WritePlaceFooter(md);
					return true;
				}
			}
	...
	per.showDancing = function()
	{
		...
		else if (img == "poledancec") md.write('<p>Catherine is wearing a stylised construction gear with a tight top as she steps up onto the stage. She is looking happy and confident as she dances and strips for you and everyone else. She is talented and really pleased to be performing!</p>');
		else md.write('<p>Catherine is wearing a stylised military uniform as she steps up onto the stage. She is looking happy and confident as she dances and strips for you and everyone else. She is talented and really pleased to be performing!</p>');
	...
	per.showPersonChat = function(md)
	{
		...
		if (checkPersonFlag("DoctorKay", 1) && !checkPersonFlag("DoctorKay", 2)) {
			addQuestionR(md, 'ask Catherine about the Nurse you saw at school',
				...
				"Catherine",
				"bChatLeft=false;setPlaceKnown(\\'NursesOffice\\',false);setPersonFlag(\\'DoctorKay\\',2)"
			);
		}			
		...
		if (perGates.checkFlag(6) && !isPlaceKnown("AvernusClub")) {
			addQuestionR(md, '"Catherine, is there a gentleman\'s club in Glenvale?"',
				...
				"Catherine",
				"bChatLeft=false;setPlaceKnown(\\'AvernusClub\\',false)"
			);
		}
	...
	per.isPhoneable = function(msg) {
		...
		if (!this.checkFlag(10)) return false;
