...
function initialisePrincipalReagan()
{
	...
	per.getPersonNameShort = function() {
		if (isAtLocation(9)) return "Principal Reagan";
		if (this.isCharmedBy()) {
			// if Charmed
			var clv = this.getCharmedLevel();
			if (clv == 4) return "Slave Reagan";
	...
	per.showPersonChat = function(md)
	{
		...
		if (Place == 71 && sType === "" && this.isHere() && this.isCharmedBy()) {
			...
			if (!isPlaceKnown("PrincipalsHouse")) {
				addQuestionR(md, 'ask Principal Reagan where she lives',
		...
		if (Place == 390 && sType === "" && this.isHere() && this.isCharmedBy()) {
			addLinkToPlaceC(md, this.isLover() ? 'have a shower with your lover' : 'have a shower with your slave', Place, 'type=reaganshower');
			...
			addLinkToPlaceC(md, 'go into the bedroom' , 391);
	...
	per.showEvent = function()
	{
		...
		if (Place == 70 && (!this.checkFlag(1) && checkPersonFlag("MrBeasley", 1) && isShopOpen(2) && sType === "") || sType == "reaganintro1"){
			...
			startQuestions();
			addLinkToPlace(md, "leave the room confused", 70, 'type=reaganintro2', 'Before you leave the room, the principal calls you back');
			WritePlaceFooter(md);
			return true;			
		}

		if (Place == 70 && sType == "reaganintro2") {
			// Part 2 of the intro
			md = WritePlaceHeader();
			...
			startQuestions();	
			addLinkToPlace(md, "leave the room to the hallway", 70);
			WritePlaceFooter(md);
			return true;				
		}
		...
		if (Place == 71 || Place == 390 || Place == 391) {
			...
			if (sType == "reagantitfuck" || sType == "showertitfuck") {
				...
				if (clv == 3) {
					...
					addPlaceTitle(md, "Fucking Your Slave\'s Tits");
					md.write('<p>You fuck your slaves tits.</p>');
				}
				startQuestions();
			...
			if (sType == "reaganfuck" || sType == "showerfuck") {
				...
				if (sType == "showerbj") this.showPersonRandomRorX("reagan-shower-malebj", isExplicit() ? 2 : 1);					
					...
					if (!isExplicit() || !perYou.isMaleSex()) {
						if (perYou.isMaleSex()) this.showPersonRandom("reagan.charm.bj", 1);
		...
		if (sType == "freemsreagan") {
			...
			startQuestions();
			if (bEarly) addLinkToPlace(md, "leave her office", 70);
		...
		if (sType == "charmprincipaloffice3") {
			...
			if (this.isLover()) {
				...
				md.write(
					...
					'<p>She immediately replied, "Okay ' + perYou.getYourNameFor() + ' you can do whatever you want to the school just let me serve you, I am your slave"</p>'
				);
				startQuestions();
		...
		if (sType == "charmprincipaloffice4") {
			...
			// Choices
			startQuestions();
			addLinkToPlace(md, "leave her office", 70);
			WritePlaceFooter(md);
			return true;
		}	
	...
	per.callThem = function() {								// Phone them
		if (Place == 269) {
			...
			else receiveCall('','You call your slave Principal Reagan and order her to join you at the pool, and immediate obeys and says she will be there as soon as she can.'); 
	...
	per.getPersonSMS = function(id) {
		switch(id) {
			case 400: return receiveSMS('SchoolAdmin', 'You have a mandatory midday meeting in the Principal\'s Office');
			case 401: return receiveSMS('MsReagan', 'Good morning ' + this.getYourNameFor() + ' I am thinking about you', "sms1.jpg");
			case 402: return receiveSMS('MsReagan', 'I thought I would try a new scarf, do you like it?', "sms2.jpg");
			case 403: return receiveSMS('MsReagan', 'Not quite balls deep...', "sms3.jpg");
			case 404: return receiveSMS('MsReagan', 'I often workout at the gym on weekends', "sms4.jpg");
			case 405: return receiveSMS('MsReagan', 'A Halloween costume from last year', "sms5.jpg");
			case 406: return receiveSMS('MsReagan', 'Another sort of costume', "sms6.jpg");
		}
		return '';
	};
