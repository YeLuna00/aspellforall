...
function initialiseMrsRobbins()
{
	...
	per.showEventPopup = function()
	{
		if (sType == "geraldinetransformbodymaria") {
			...
			showPopupWindow("Transformed",
				this.addPersonString("robbins5.jpg", "height:max%", "right") +
		...
		if (Place == 176 && !this.checkFlag(2)) {
			// See Mrs Robbins
			if (this.dress === "") {
				this.pickModel("You have only met Mrs. Robbins, the mother of your nemesis Davy once before. Does she have...", "robbins0", "Maria", "Charley", "brown hair", "black hair", '', "Mrs. Robbins");
			} else {
		...
		if (sType == "geraldineprivate") {
			...
			if (!perTina.isHere()) {
				this.addSleepLink(md, "go to bed for the night with Geraldine", "Bedding Geraldine",
					...
					"robbins12a.jpg", false, 82, '', 'When you wake in the morning Geraldine has left to prepare breakfast.', undefined, undefined, '25%'
				);
			}
	...
	per.showPersonTextHere = function(md)
	{
		if (Place == 176 && this.isHere() && !isInvisible()) {
			...
			md.write('</p>');
		} else if (Place == 82 && sType === "" && this.isCharmedBy() && isCharmedBy("Tina") && isVisible()) md.write('<p>You see Geraldine and Tina watching you from the doorway, not entering as you have not invited them in.</p>');
	};
