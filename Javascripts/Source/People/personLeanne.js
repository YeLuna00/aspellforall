...
function initialiseLeanne()
{
	...
	per.getPossessionFace = function() { return "leanne-face" + (this.isCharmedBy() ? "c" : (this.isCharmedBy("Demon") ? "t" : "u")); };	
	...
	per.showEventPopup = function()
	{
		...
		if (Place == 384 && this.place == 450 && !this.checkFlag(27) && this.isCharmedBy("Demon")) {
			...
			showPopupWindow("Where is Leanne?",
				this.addPersonString("leanne8t.jpg", "height:max%", "right") +
	...
	per.showEvent = function()
	{
		...
		if (Place == 269) {
			if (sType == "leannepool") {
				WaitHereOnly(6);
				md = WritePlaceHeader();
				this.showPerson("pool" + (this.isCharmedBy() ? "c" : "u") + ".jpg");
				addPlaceTitle(md, "Swimming with Leanne");
				md.write(
					'<p>Leanne arrives and changes into her swimsuit and happily wades out into the pool with you.</p>'
				);
				startQuestions();
		...
		if (sType == "fuckthrallleanne") {
			md = WritePlaceHeader();
			this.showPersonBG("leanne14t.jpg");
		...
		if (sType == "oralservice") {
			...
			else AddImageGM("GenericSex/sex-mf blowjob-brown hair b.jpg");
	...
	per.showPersonChat = function(md)
	{
		if (Place == 195 && this.isHere() && sType === "") {
		...
		else if ((Place == 382 || Place == 384) && this.isHere() && sType !== "take2"  && sType !== "take") {
			// Is the thrall here
			...
			if (isPossess()) addLinkToPlace(md, 'take the thrall', Place, 'type=take');
