...
function initialiseLouise()
{
	...
	per.getPossessionFace = function() { return this.isCharmedBy() ? "louise-facec" : "louise-faceu"; };
	...
	{
		if (Place == 463 && this.isHere() && sType === "") return this.showPerson("home1" + this.getSuffix() + ".jpg", '', '', '', '', false, "string");
		else if (Place == 195 && this.isHere() && sType === "") {
			if (this.isCharmedBy()) return this.showPerson("louise10" + this.getSuffix() + ".jpg", '', '', '', '', false, "string");
	...
	per.showEvent = function()
	{
		...
		if (sType == "charmlouise3") {
			...
			WritePlaceFooter(md, "Script by Tilde, adapeted for lover path");
			return true;
		}
	...
	per.showPersonChat = function(md)
	{
		if (Place == 195 && this.isHere()) {
			if (this.isCharmedBy() && sType === "") {
				addLinkToPlace(md, 'risk your ' + (this.isLover() ? 'lover' : 'slave') + '\'s job again', 195, 'type=riskjob');		//Louise is charmed
