...
function initialiseZali()
{
	...
	per.getPersonName = function(full) {
		...
		if (clv == 3) return "MILF Lover";
		if (clv == 4) return "MILF Slave";
		return this.name;
	};
	...
	per.showEvent = function()
	{
		...
		if (Place == 269) {
			if (sType == "zalipool") {
				...
				startQuestions();
				addLinkToPlaceC(md, 'it is fairly private here...', Place, 'type=zalipoolsex');
		...
		if (sType == "charmzalilover2") {
			...
			md.write(
				'<p>As the spell takes ahold of her, she looks more flustered and says, "Oh my I feel so hot… I haven’t felt this way since Richard passed, oh my…"</p>' +
		...
		if (sType == "charmzalilover3") {
			...
			md.write(
				'<p>“Opps, my robe slipped!” she smiles in a seductive way as she looks at you.</p>' +
		...
		if (sType == "zalifuck") {
			...
			startQuestions();
			addLinkToPlaceC(md, 'talk more to Zali', Place);
			WritePlaceFooter(md);
			return true;
		}
		...
		if (sType == "zalibj") {
			...
			addPlaceTitle(md, "Oral sttention from your " + (clv == 4 ? "slave" : "lover"));

			md.write(
				'<p>Zali ' + (perYou.isMaleSex() ? 'gives you a blowjob' : 'licks you') + '</p>'
			);
