...
function initialiseMsJones()
{
	...
	per.getPossessionFace = function() { return this.isCharmedBy() ? "jones2" : "!jones-face"; };
	...
	per.showEvent = function()
	{
		...
		if (Place == 269) {
			...
			if (sType == "msjonespoolsex") {
				md = WritePlaceHeader();
				if (isExplicit() && perYou.isMaleSex()) this.showPersonRandomX("!pool-sexb", 2);
				else this.showPersonBG("!pool-sex.jpg");
	...
	per.showDancing = function()
	{
		...
		var ap = Math.random() < 0.5;
		if (ap) this.showPerson("!poledancea.jpg");
		else this.showPerson("!poledanceb.jpg");
		addPlaceTitle(md, "Ms. Jones' Dance");
		if (ap) md.write('<p>Ms. Jones is wearning a sot os stylised gangster outfit and some a skilled strip-tease. She is very focused and an excellent stripper but you are briefly distracted by the thought that she could have dressed this way during the actual era she is dressed for...</p>');
		else md.write('<p>Ms. Jones is wearning a costume, a fantasy take on Catwoman, then again is any Catwoman costume worn for anthing else. She is an excellent dancer and can really handle that whip!</p>');
