...
function initialiseSarah()
{
	...
	per.showEvent = function() {
		...
		if (sType == "endgame1sarah") {
			// End Game - Sarah (Charmed)
			md = WritePlaceHeader();
			this.showPerson("pregnantc.jpg");			
			addPlaceTitle(md, "A Very Contagious Lesson for Heiress's?");

			md.write(
				'<p>One day you visit the lovely Sarah and she lits her top showing her swollen pregnant belly'
			);
			if (isCharmed("Lauren")) md.write(' and you see her glance over at the door. There you see Lauren also with a pregant belly looking a little embarrassed');
			md.write('. It seems you learnt the lessons on Miss. Logan only too well.</p>');
			...
			if (isCharmed("Lauren")) {
				AddPeopleColumnLarge(md);
				findPerson("Lauren").showPerson("pregnant.jpg");
			}
			WritePlaceFooter(md);
			return true;				
		}
	...
	{
		return this.isCharmedBy() ? "endgame1sarah" : "";
	};
