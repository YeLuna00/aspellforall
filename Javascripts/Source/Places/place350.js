...
function ShowPlace350()
{
	...
	AddImage("cherryitems.jpg", "20%", "right");

	/* Description */
	md.write(
		'<p>The Chakra Chachkies ' + getShopStore(true) + ' looks like a quaint little ' + getShopStore() + ' in an older building. On the inside you see a modern looking yoga studio with a small area selling a variety of new age items, with a definite emphasis on crystals.</p>'
	);
	if (isVisible()) {
		if (!perCherry.checkFlag(8) && !perCherry.isCharmedBy()) {
			...
			//CHARMED
			md.write(
				'<p>Only people who are interested in yoga or new age crystal power visit here, and it seems there are few such people in Glenvale. The studio stands mostly empty during the day, only one or two costumer stop by.</p>'
			)
			if (med) md.write('<p>You notice Cherry is mediating when you enter the store.</p>');
			if (clv == 3) {
				md.write(
					'<p>When your lover Cherry sees you enter she removes much of what she is wearing, her lust rising as you approach. You and not completely sure this is because of the spell!</p>' +
					'<p>"Hello lover. I\'m so glad you\'re here, let\'s get sweaty together!"</p>'
				);				
			} else {
				md.write(
					'<p>Your lovely slave Cherry is here and she bows to you as you enter and says,</p>' +
					'<p>"Hello, ' + perYou.getMaster() + '. I\'m so glad you\'re here. Is there <i>anything</i> I can do for you?"</p>' +
