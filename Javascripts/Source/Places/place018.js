...
function ShowPlace18(stype)
{
	...
	if (perGates.other === 0 && !perGates.checkFlag(10)) {
		// First Visit
		if (perLauren.dress === "") {
			perLauren.pickModel("You see a young woman in a maid outfit open the door. For a moment you are confused. Is she...", "lauren1a2col-day", "Shay", "Holly", "shorter", "taller", '', "A Maid?");
			WritePlaceFooter(md);
			return;	
		}
		perLauren.showPersonDN("lauren1b.jpg");
		addPlaceTitle(md, "Gates Mansion");
		perGates.setFlag(10);
		md.write(
			...
			'<p>You give your name and ask to see ' + perGates.getPersonName() + ' to discuss a book called the Sacred Book of Control. She excuses herself and closes the front door. A short time later she returns and invites you in.</p>' +
			'<p>As you follow her she notices your glances at her skimpy dress, and she blushes,' +
			...
			'<p>With some embarrassment she leads you towards a large library, but leaves you at the door and gestures for you to enter.</p>'
		);

		// Questions
		startQuestions();
		addLinkToPlaceC(md, "follow her to " + perGates.getPersonName(), 17);
		WritePlaceFooter(md);
		return;		
		...
		if (perSarah.other == 1000 || perSarah.place != 192) md.write(' a maid dressed rather scantily,');
	...
	} else if (perSarah.place == 192 && (perSarah.other >= 114 && perSarah.other <= 130 && ((perSarah.other - 114) % 3 == 0))) {
		// Meet Lauren later
		if (perLauren.dress == "") perLauren.dress = "Shay";	
		perLauren.showPersonDN("lauren1b.jpg");
		addPlaceTitle(md, "Met in the Entry Hall");
