...
function Person(nm, plc, fldrin, drs, dance)
{
	...
	this.getYourNameFor = function() {
		...
		if (clv <= 0 || clv == 1 || clv == 3) return perYou.getPersonName();
		if (clv == 2 || clv == 3) return "my Love";
	...
	this.whereNow = function(mid) { return this.place; };		// 'virtual' for abstract locations like 'home' for some people
	...
	this.showPersonRandomBG = function(imgbase, no, wid, alg, imgbig, title, baseno, dn, doc) { return AddImageRandom(this.getImg(addBGSuffix(imgbase), dn), no, wid, alg, this.getImg(addBGSuffix(imgbig), dn), !title ? this.getPersonName() : title, baseno, this, doc); };
	...
	this.showPersonRandomXBG = function(imgbase, no, wid, alg, imgbig, title, baseno, dn, doc) { return AddImageRandom(this.getImg("Explicit/" + addBGSuffix(imgbase), dn), no, wid, alg, this.getImg(imgbig, dn), !title ? this.getPersonName() : title, baseno, this, doc); };
	...
	this.showPeopleRandom = function(perOther, imgbase, no, wid, alg, imgbig, title, baseno, dn, doc) {
		if (typeof perOther == "string") perOther = findPersonNC(perOther);
		return AddImageRandom(this.getImg(perOther.getImgS(imgbase, dn), dn), no, wid, alg, this.getImg(perOther.getImgS(imgbig, dn), dn), !title ? this.getPersonName() + " " + perOther.getPersonName() : title, baseno, this, doc);
	};
	this.showPeopleRandomX = function(perOther, imgbase, no, wid, alg, imgbig, title, baseno, dn, doc) {
		if (typeof perOther == "string") perOther = findPersonNC(perOther);
		return AddImageRandom(this.getImg("Explicit/" + perOther.getImgS(imgbase, dn), dn), no, wid, alg, this.getImg(perOther.getImgS(imgbig, dn), dn), !title ? this.getPersonName() + " " + perOther.getPersonName() : title, baseno, this, doc);
	};
	...
	this.addPersonFace = function(esc, wid) {
		...
		if (!wid) wid = "20%";
		if (esc === true) return '<img src=\\\'' + gameState.getImagesFolder() + img + '.jpg\\\' style=\\\'float:left;width:' + wid + ';margin:0 10px 2em 0\\\' alt=\\\'' + this.uid + '\\\' title=\\\'' + this.getPersonName() + '\\\'>';
		return '<img src="' + gameState.getImagesFolder() + img + '.jpg" style="float:left;width:' + wid + ';margin:0 10px 2em 0" alt="' + this.uid + '" title="' + this.getPersonName() + '">';
	};
	...
	this.pickModel = function(txt, img, drs1, drs2, chc1, chc2, stype, tit) {
		showPopupWindow(tit !== undefined ? tit : "Someone is there",
			...
			: "<img src='Images/People/" + this.folder + "/" + this.getDress('', drs1) + "/" + img + ".jpg' style='float:left;width:30%;max-height:100%;height:auto;margin-right:5px' alt='Who' title='" + img + "'>" +
	...
	this.pickModelMore = function(txt, img, stype, tit) {
		...
		for (i = 0; i < ar.length; i++) {
			...
			if (isScreenSmall()) {
				...
				s += "<td style='vertical-align:top'><a href='javascript:findPerson(\"" + this.uid + "\").dress=\"" + ad[0] + "\";dispPlace(" + Place + (stype !== "" ? ",\"type=" + stype + "\"" : '') + ")'><img src='Images/People/" + this.folder + "/" + this.getDress('', ad[0]) + "/" + img + ".jpg' style='float:left;width:99%;max-height:100%;height:auto;margin-right:5px' alt='Who' title='" + img + "'></a></td>";
			}
		}
		if (isScreenSmall()) {
			//console.log('small');
	...
	this.perMod = undefined;											// Linked list of mods	
	...
	this.getPersonAddress = function(n) { return n ? 0 : ""; };		// Their address
	...
	this.makeCall = function(bSMS, from, txt, eok) {
		...
		if (bSMS) {
			...
			if (this.checkFlag(64) && !this.isSMSImportant(from)) return true;		// SMS blocked but flagged as sent
			addSMS(from);
			bNewSMS = true;
			nUnreadSMS = from;
	...
	this.messageThemBase = function(type) {								// SMS them, default as if you can always message them
		if (type === "where") {
			var w = this.whereNow();
			var wm = this.getPersonAddress(true);
			var nm;
			if (w == -1) nm = "right here with you";
			...
			var img = getImageO(this.isCharmedBy() ? "smssexy" : "smsselfie", -9, 0, this);
			if (img === "") img = getImageO(this.isCharmedBy() ? "smsselfie" : "smssexy", -9, 0, this);
			if (img === "") img = getImageO("poledance", -1, 0, this);
	...
	this.addDancingCall = function(txt) {
		...
		perJade.setDancer(this.uid);
		dispPlace();			
		receiveCall('', txt !== undefined ? txt : ('You call ' + this.getPersonName() + ' and ask ' + this.getHimHer() + ' to join you at the Avernus club to dance for you. You make arrangements with Jade for the dance.'));
		WriteCommentsFooter(bChat, bChatLeft);	
	}	
	...
	this.addSleepLink = function(md, lnk, title, body, img, white, plc, params, txt, sty, cnt, wid) {
		...
		addSleepLink(md, lnk, title, body, this.getImg(img), white, plc, params, txt, sty, cnt, this, wid);
		/*
...
function ShowPopupEvents()
{
	...
	if (getQueryParam("showinventory") !== "") {
		...
		return;		
	} else if (getQueryParam("playagame") !== "") {
	...
	if (gameState.arNewPlaces !== '') {
		...
		if (ar.length > 0) {
			playSound('sms.mp3');
			setComments("Your phone chimes that " + (ar.length > 1 ? "new locations<br>are" : "a new location<br>is") + " available on your map:");
		}
...
function buildModelRow(p)
{
	...
	var s = '<td>' + p.getPersonName() + '</td><td style="width:10vw;vertical-align:top"><a onclick="selthis(this)">' + sn + '<img src="' + getThemeFolderI(1) + 'unknown.png" style="width:90%;' + (p.dress === '' ? 'border:5px solid yellow' : 'border:5px solid black') + '"></a></td>';
	var ar = m.split(",");
	var o = p.dress;
	...
	for (j = 0; j < ar.length; j++) {
		arThis = ar[j].split("|");
		p.dress = arThis[0];
		var img = p.addPersonFace(false,'90%;' + (o == arThis[0] ? 'border:5px solid yellow' : 'border:5px solid black'));
		...
		if (sn !== '') {
			for (i = 0; i < prev.length; i++) {
				if (arThis[1] == prev[i]) {
					oc = '';
					img = '<img src="UI/noicon.png" style="width:90%;border:5px solid black"/>';
					//console.log(img);
					break;
				}
			}
		}
		s += '<td id="' + arThis[0] + '" style="width:10vw;vertical-align:top">' + oc + sn + img + (oc !== '' ? '</a>' : '') + '</td>';
		p.dress = o;
	}
	return s;
}
...
function selectModels(drs, sh)
{
	...
	var s =	'<img src="Images/People/Bambi/bambipick.jpg" class="imgpopup" style="width:30%"/>' +
				'<p>You may select the models used in the game, except where they are preselected by a mod, or there is no choice! Select the unknown icon (<img src="' + getThemeFolderI(1) + 'unknown.png" style="width:32px">) to choose when you first meet the character. </p>' +
	...
	for (i = 0; i < par.length; i++) {
		p = findPerson(par[i]);
		s += '<tr id="' + p.uid + '">' + buildModelRow(p) + '</tr>';
	}
