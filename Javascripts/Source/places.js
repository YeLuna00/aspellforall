...
function setPlaceKnownBase(ps, notify)
{
	...
	var idx = getPlaceIdx(ps);
	if (idx !== 0) {		
		if ((arPlaces[idx] & 1) === 0) gameState.arNewPlaces = gameState.arNewPlaces + (gameState.arNewPlaces !== "" ? "," : "") + getPlaceName(idx);
	...
	if (!isPlaceKnown(ps) && notify !== false && ps.indexOf("Apartment") == -1 && ps.indexOf("House") == -1) gameState.arNewPlaces = gameState.arNewPlaces + (gameState.arNewPlaces !== "" ? "," : "") + ps;
...
function gotoPlace(plc, params, txt, ps, click)
{
	Leave(false);
	// Extra time?
	var stype = getQueryParam("type", params);
...
function WritePlaceFooter(doc, credits)
{
	...
	if (!gameState.bNoPeople) ShowPopupEvents();
