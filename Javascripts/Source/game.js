...
function mainCache()
{
	...
	this.finaliseRightCol = function()
	{
		...
		if (!gameState.bDisableRightCol) {
			...
			var sz = gameState.sRightColSize == "td-right" ? "20%" : (gameState.sRightColSize == "td-right-med" ? "30%" : "35%");
...
function getVersion() { return "14.13c"; }
...
function GameStatus()
{
	...
	this.nSMSLimit = 13 * 32;		// CONSTANT: Must be multiple of 32 and never changes in gameplay
...
function getImageO(imgbase, noi, baseno, per, alpha, next)
{
	...
	if (typeof no == "string" || typeof no == "object") {
		...
		if (ar.length !== 0) {
			...
			if (imgbase.indexOf("-day") != -1) return imgbase.split("-day")[0] + "-day" + (ext === undefined ? '.jpg' : ext);
			else if (imgbase.indexOf("-night") != -1) return imgbase.split("-night")[0] + "-night" + (ext === undefined ? '.jpg' : ext);
			return imgbase + (ext === undefined ? '.jpg' : ext);
		} else no = 0;
	}
	if (no === 0) {
		if (noi === -9) return '';
...
function addImageString(img, widh, alg, imgbig, title, per, ph)
{
	...
	// base64 version
	if (ph !== 'noall') st += '<a draggable="false" style="height:100%;display:inline-block;position:relative;' + (alg == "center" ? widh : 'width:100%') + '" href="' + imgbig + '" onclick="onclickImage(this)"' + (img.indexOf(".mp4") != -1 || isScreenSmall() ? '>' : ' rel="lightbox">');
	var tp = 0;
...
function onerrorImage(el)
{
	...
	el.onerror = function() {
		...
		myVid.innerHTML = '<video id="viderr" autoplay muted loop style="' + (wd != '' ? 'width:' + wd + ';' : '') + (ht != '' ? 'height:' + ht + ';' : '') + 'max-width:100%;margin:0px 0px' + (ml !== "" ? ';margin-left:' + ml : '') + '"' + sAuto + '><source src="' + imgv + '" type="video/mp4" onerror="onerrorVideo(this)" data-img="' + imgl + '" data-oimg="' + imgo + '"></video>';
		...
		for (var i = 0; i < ppn.childNodes.length; i++) {
			...
			if (curChild.nodeType == Node.ELEMENT_NODE && curChild.tagName.toLowerCase() === "img") {
				...
				if (imgv.indexOf("Mods/") != -1 || imgv.indexOf("Explicit/") != -1 || imgv.indexOf("Younger/") != -1) {
					...
					var ext = ar[ar.length - 1].split('#')[0];	// Extension of the image
					if (ext === null) ext = "jpg";
					curChild.src = imgv.split('.gif').join('.' + ext).split('.jpg').join('.' + ext).split('.mp4').join('.' + ext);
...
function addBGSuffix(img)
{
	...
	var idx = ar.length - 1;
	var ext = ar[idx];
	if (ar.length > 1 && ['jpg', 'mp4', 'gif', 'png'].indexOf(ext) >= 0) idx--;
...
function addPopupWindow(md, title, txt, onclose, sty, blk, noclick, nocloseb, sb, idin)
{
	...
	st += "sOldPlaceParams=sPlaceParams;var fl=document.getElementById('light" + id + "');if(fl){fl.style.display='none\'};setQueryParams();showSideBars();bPopupShown=false;";
	...
	s += "</b></p></div>" +
		"<div id='cheat'" + (gameState.bCheatShown ? "" : " style='display:none'") + ">" +
			...
			"<form method='GET' id='Cheating' action='.'>" +
				...
				"<a href='' onclick='toggleExplicit();updateLeftBar();return false' title='Explicit Images'>Explicit Scenes <b>" + (isExplicit(true) ? "On" : "Off") + "</b></a>" +
				...
				"<br><a href='' onclick='toggleTaxis();updateLeftBar();return false'>" + (bTaxiAlways ? "Taxis common" : "Taxis less common") + "</a>" +
				...
				"<br><a href='' onclick='dispPlace(Place,\"type=selectmodels\");return false'>Select Models</a>" +				
